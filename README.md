...
- **Analysis:** `exploration/pockets_build_summary.py` — Weibull return
  ...
  median RP of the four rainfall witnesses; vegetation pillar = RP of the
  linearly detrended regional VHI, raw ASI kept as context;
  JRC-CDI-style classes), reconstructed for every season 1991–2026.
  CERF drought seasons (2009, 2011, 2021 — mapped to the drought's
  valid growing season from `aa.cerf_allocation`; the 2022 AA allocation
  excluded, and the Sep 2008 "drought" allocation excluded as a
  food-price/nutrition response per its `aa.cerf_project` records) and
  EM-DAT drought events since 2000 anchor the backtest.
...
## HCT briefing deck (El Niño & the 2026 season)

An 8-slide (plus backup) bilingual (EN/FR toggle) HTML deck at
[`/hct-brief/`](https://ocha-dap.github.io/ds-aa-ner-drought/hct-brief/)
(`docs/hct-brief/index.html`, print-to-PDF for distribution), generated by
`exploration/hct_brief_page.py`. Reuses the pockets figures plus the team's
published Niger ENSO slides from the sibling `ds-seas5-skill` clone
(`pages/enso/slides/NER_slide{1,2}[_fr].svg`). Narrative: El Niño's direct
teleconnection to Niger is weak — the concern rests on this season's own
